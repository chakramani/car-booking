...
function paradise_date_picker_shortcode_driver()
{
    if (is_user_logged_in()) {
        ...
        if (isset($_POST['block'])) {
            ...
            $data = array('user_id' => $current_user_id, 'blocked_date' => $block_date, 'status' => 'block', 'block_type' => 'self');
            $format = array('%d', '%s', '%s', '%s');
            $wpdb->insert($table, $data, $format);
            // } else {
            //     $block_date = $_POST['multi_data'];
            //     $wpdb->query($wpdb->prepare("UPDATE $table SET blocked_date='$block_date', block_type=%s WHERE user_id =%d AND status=%s", 'self', $current_user_id, 'block'));
            // }
        }
...
function get_block_date_of_specific_user()
{
    ...
    $current_user_id = get_current_user_id();
    $table = $wpdb->prefix . 'car_booking';
    // $data = $wpdb->get_results("SELECT blocked_date,block_type FROM $table WHERE `user_id` = $current_user_id AND `status`='block' ORDER BY id DESC LIMIT 1");
    $data = $wpdb->get_results("SELECT blocked_date,block_type,source,destination FROM $table WHERE `user_id` = $current_user_id AND `status`='block'");
    return $data;
}
...
function book_date_range_for_car_booking()
{
    ...
    if (empty($bookings[0]->{'blocked_date'})) {
        ...
        $assigned = array('user_id' => $current_user_id, 'date_from' => $starting_date, 'date_to' => $ending_date, 'assigned_user' => $random_id, 'source' => $source, 'destination' => $destination, 'no_of_travellers' => $no_of_travellers, 'status' => 'booking', 'block_type' => 'booking');
        $assigned_format = array('%d', '%s', '%s', '%d', '%s', '%s', '%d', '%s', '%s');
        ...
        // if ($result) {
        //     $add_block_id = $wpdb->get_results("UPDATE $table SET `blocked_date` = CONCAT(blocked_date, ', $concat_ids'),block_type = 'booking'  WHERE `user_id` = $random_id");
        // } else {
        $data = array('user_id' => $random_id, 'blocked_date' => $concat_ids, 'status' => 'block', 'block_type' => 'booking', 'source' => $source, 'destination' => $destination);
            ...
            if (($pagenow == 'user-edit.php' || $pagenow == 'profile.php') && in_array('driver', $user_role)) {
                ...
                <table class="form-table">
                    <tr>
                        ...
                        <td>
                            <div class="driver_car_image_main">
                                <iframe src="<?php echo $ps_video_url; ?>"> </iframe>
                                <input type='url' name='ps_video_id' id='ps_video_id' value='<?php echo $ps_video_url ? $ps_video_url : ''; ?>' placeholder='URL...' />
                            </div>
                        </td>
                    </tr>
                </table>
